...
def draw_risk_bar(score):
    ...
    d.add(
        Rect(0, 12, fill_width, 10, fillColor=bar_color, strokeColor=None, rx=5, ry=5)
    )
    d.add(
        Line(
            157.5, 10, 157.5, 24, strokeColor=colors.HexColor("#94A3B8"), strokeWidth=1
        )
    )
...
def get_recommendations(disease_type, risk_percent, inputs):
    ...
    if risk_percent < 35:
        recs.append(
            "<b>General Assessment:</b> Low risk score detected. Patient parameters indicate normal ranges. Continue current lifestyle choices and undergo standard annual wellness screenings."
        )
    elif risk_percent < 70:
        recs.append(
            "<b>General Assessment:</b> Moderate risk score detected. Patient exhibits mild anomalies. Clinical observation and proactive lifestyle adjustments are recommended to mitigate risk factors."
        )
    else:
        recs.append(
            "<b>General Assessment:</b> High risk score detected. Patient displays significant risk parameters that correlate strongly with pathological indicators. A comprehensive physician checkup is advised as soon as possible."
        )
    if disease_type == "Diabetes":
        if risk_percent >= 35:
            recs.append(
                "Monitor blood glucose levels and schedule an HbA1c test in 3 months to establish baseline tracking."
            )
            recs.append(
                "Reduce intake of high glycemic index (GI) carbohydrates, saturated fats, and refined sugars."
            )
        if float(inputs.get("bmi", 0)) >= 25.0:
            recs.append(
                f"Current BMI ({inputs['bmi']} kg/m²) indicates overweight/obesity range. Aim for moderate weight management through structured exercise (minimum 150 mins per week)."
            )
        if inputs.get("hypertension") == 1 or inputs.get("heart_disease") == 1:
            recs.append(
                "Co-existing hypertension or cardiovascular disease increases metabolic risk factors. Coordinate care between cardiological and primary care professionals."
            )
        if inputs.get("smoking_history") in [
            "current",
            "former",
            "ever",
            "not current",
            ...
            recs.append(
                "Maintain strong smoking cessation. Tobacco exposure negatively affects vascular health and insulin sensitivity."
            )
    elif disease_type == "Heart Disease":
        if risk_percent >= 35:
            recs.append(
                "Schedule a comprehensive lipid panel to assess LDL, HDL, and triglyceride levels in detail."
            )
            recs.append(
                "Adopt a heart-healthy diet such as the DASH or Mediterranean diet, limiting sodium intake to under 2,000 mg per day."
            )
        if float(inputs.get("systolic_bp", 0)) >= 130.0:
            recs.append(
                f"Systolic Blood Pressure ({inputs['systolic_bp']} mmHg) is in the elevated/hypertensive range. Monitor daily at home and record values for review."
            )
        if float(inputs.get("cholesterol", 0)) >= 200.0:
            recs.append(
                f"Total Cholesterol ({inputs['cholesterol']} mg/dL) exceeds normal levels. Focus on soluble dietary fiber and consult about therapeutic options if values remain elevated."
            )
        if float(inputs.get("bmi", 0)) >= 25.0:
            recs.append(
                f"Elevated BMI ({inputs['bmi']} kg/m²) places additional mechanical and metabolic stress on the heart. Consider active weight management."
            )
        if inputs.get("previous_heart_attack") == 1:
            recs.append(
                "History of previous heart attack is a critical risk factor. Ensure adherence to prescribed beta-blockers, antiplatelet, or statin therapies under supervision."
            )
    elif disease_type == "Stroke":
        if risk_percent >= 35:
            recs.append(
                "Monitor blood pressure daily; stroke risk is strongly correlated with poorly controlled hypertension."
            )
            recs.append(
                "Screen for sleep apnea or sleep architecture issues, which are recognized independent contributors to stroke risk."
            )
        if inputs.get("high_blood_pressure") == 1:
            recs.append(
                "Diagnosed hypertension needs aggressive control. Work with your physician to optimize medication management."
            )
        if inputs.get("chest_pain") == 1 or inputs.get("irregular_heartbeat") == 1:
            recs.append(
                "Active cardiorespiratory symptoms (Chest Pain, Irregular Heartbeat) require urgent investigation (e.g. ECG) to rule out atrial fibrillation or coronary insufficiency."
            )
        if inputs.get("dizziness") == 1 or inputs.get("shortness_of_breath") == 1:
            recs.append(
                "Reported dizziness and dyspnea may indicate transient cerebral hypoperfusion or circulatory strain. Rest and contact your healthcare provider if persistent."
            )
    return recs
...
def generate_pdf_report(patient_info, prediction_info, inputs, metrics_list):
    ...
    bold_style = ParagraphStyle(
        "ReportBold", parent=normal_style, fontName="Helvetica-Bold"
    )
    ...
    table_text_style = ParagraphStyle(
        "TableText", parent=normal_style, fontSize=9, leading=12
    )
    ...
    banner_data = [
        ...
        [
            Paragraph(
                f"PREDICTIVE ANALYSIS & HEALTH PROFILE &bull; {prediction_info['type'].upper()} SCREENING",
                subtitle_style,
            )
        ],
    ]
    ...
    meta_data = [
        [
            ...
            Paragraph(patient_info["name"], normal_style),
            Paragraph("<b>Date of Report:</b>", normal_style),
            Paragraph(patient_info["date"], normal_style),
        ],
        [
            Paragraph("<b>Patient ID:</b>", normal_style),
            Paragraph(patient_info["patient_id"], normal_style),
            Paragraph("<b>Assessment:</b>", normal_style),
            Paragraph(f"{prediction_info['type']} Risk", normal_style),
        ],
        [
            Paragraph("<b>Methodology:</b>", normal_style),
            Paragraph("Machine Learning Classification", normal_style),
    ...
    badge_style = ParagraphStyle(
        "Badge", parent=bold_style, fontSize=12, textColor=colors.white, alignment=1
    )
    score_content = [
        [
            [
            ...
            Table(
                [[Paragraph(risk_level.upper(), badge_style)]],
                colWidths=[140],
                style=[
    ...
    def add_page_decorations(canvas, doc_template):
        ...
        canvas.drawString(
            54, 30, "CONFIDENTIAL &bull; AI MULTI-DISEASE ANALYSIS CLINICAL REPORT"
        )
